    ...
    builder.Services.AddHangfire(config =>
        config.UseStorage(
            new MySqlStorage(
                ...
                {
                    TransactionIsolationLevel = System.Transactions.IsolationLevel.ReadCommitted,
                    QueuePollInterval = TimeSpan.FromSeconds(15),          // Check queue every 15s
                    JobExpirationCheckInterval = TimeSpan.FromHours(6),
                    CountersAggregateInterval = TimeSpan.FromMinutes(15),
                    PrepareSchemaIfNecessary = true,
                    DashboardJobListLimit = 50000,
                    TransactionTimeout = TimeSpan.FromMinutes(5),          // Max 5 min for DB transactions
                    InvisibilityTimeout = TimeSpan.FromMinutes(5)          // Job timeout (similar to SlidingInvisibilityTimeout in SQL Server)
                }
            )
        )
    );

    // Add Hangfire Server with optimized BackgroundJobServerOptions
    builder.Services.AddHangfireServer(options =>
    {
        options.WorkerCount = 5;                                      // Number of concurrent jobs
        options.ServerTimeout = TimeSpan.FromMinutes(5);              // Max job execution time
        options.ShutdownTimeout = TimeSpan.FromSeconds(30);           // Graceful shutdown time
        options.SchedulePollingInterval = TimeSpan.FromSeconds(15);   // How often to poll for scheduled jobs
    });
    // ================================================
    // ================================================
