		...
		public async Task<IActionResult> SavePatientRecord([FromBody] ClinicPatientRecordCreateRequest request)
		{
			...
			try
			{
				...
				else
				{
					...
					if (request.Reference_Id.HasValue && request.Reference_Id.Value > 0)
					{
						...
						if (referencedRecord != null && !string.IsNullOrWhiteSpace(referencedRecord.JsonData))
						{
							try
							{
								...
								// If creating Invoice and referencing Treatment OR MembershipPlan, extract pre-generated invid
								if (parsedType == RecordType.Invoice &&
									(referencedRecord.Type == RecordType.Treatment || referencedRecord.Type == RecordType.MembershipPlan))
								{
									if (refJsonDoc.RootElement.TryGetProperty("patient", out var patientObj))
									{
										if (patientObj.TryGetProperty("invid", out var invIdProp))
										{
											...
											_logger.LogInformation("Found pre-generated Invoice ID {InvoiceId} in {RecordType} {RecordId}",
												preGeneratedInvoiceId, referencedRecord.Type, referencedRecord.UniqueRecordId);
										}
									}
								}
					...
					// ✅ UPDATED: Both Treatment AND MembershipPlan generate invoice IDs
					if (parsedType == RecordType.Treatment || parsedType == RecordType.MembershipPlan)
					{
						...
						using (var writer = new Utf8JsonWriter(stream, new JsonWriterOptions { Indented = false }))
						{
							...
							foreach (var property in jsonDoc.RootElement.EnumerateObject())
							{
								if (property.Name == "patient")
								{
									...
									// ✅ NEW: MembershipPlan also gets invoice ID
									else if (parsedType == RecordType.MembershipPlan)
									{
										propertiesToSkip.Add("invid");
									}
									...
									// ✅ NEW: MembershipPlan gets invoice ID (no treatment ID)
									else if (parsedType == RecordType.MembershipPlan)
									{
										writer.WriteString("invid", generatedInvoiceId);
									}
